...
c*******************************************************************
      ...
      if (seed.eq.0) then
        ...
        write(*,*) 'inirng:info: 64-bit RNG from PENELOPE;'
        write(*,*) '             seeds 1 and 2 ,ICNT are:'
        write(*,'(1x,3(i11,1x))') iseed1,iseed2,icnt
      else
        seed = iseed1
      endif
      end
...
c
      ...
      DIMENSION P(NPUNT,NPUN1),F(NPUN1),X0(NPUNT),X(1)
      COMMON/RSEED/RND

com: Validate dimensions...
      if (n.gt.npunt) then
        write(*,*) 'smplx Error: array too big; enlarge npunt'
        stop
      endif
...
C+++++++++++++++++++
  ...
  208 CONTINUE
      IF(NSTEP.EQ.NITER) then
        write(*,*) 'smplx Warning: Max No. Iterations reached'
        GO TO 601
      endif
